...
**Limite connue découverte pendant ce test, non corrigée (hors périmètre de ce fix)** : le compte `admin@izigsm.fr` a `boutique_id: null` en base (confirmé en prod). Le frontend (`tickets.js`) n'envoie jamais de `boutique_id` explicite sur `GET`/`POST /api/tickets/:id/photos` ni sur `GET .../photos/:photoId/url` (pas d'auto-injection côté `apiGet` puisque `session.boutique_id` est `null` pour ce compte) — un admin naviguant normalement dans l'UI reçoit donc désormais `403` sur ces 3 endpoints photos, comme c'était déjà silencieusement le cas pour `/url` depuis son déploiement le 2026-07-15 (jamais remarqué, probablement parce que l'admin n'utilise pas ce compte pour la gestion quotidienne des photos). Pas une régression introduite par ce fix — juste rendu visible/cohérent sur les 3 endpoints. À traiter séparément si ça devient gênant en usage réel (ex. sélecteur de boutique actif pour les comptes admin globaux).
