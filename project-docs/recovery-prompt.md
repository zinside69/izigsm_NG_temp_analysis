# Recovery Prompt — iziGSM — 2026-07-12 (checkpoint 5)
...
- **7/20 services migrés** (voir `todo.md` pour la liste complète et l'ordre) : `userService` (session précédente), `photosService`, `publicService`, `boutiqueService`, `rachatService`, `personnelService`, `caisseService`, `factureService`
...
- `tests/helpers/mockDatabase.ts` — mock du port Database (ajout de `__setNotFound` aujourd'hui)
- `project-docs/todo.md` — ordre de migration complet des 13 services restants + items fuseau horaire
- `project-docs/bugs.md` — détail des 2 bugs découverts et corrigés aujourd'hui
...
23 fichiers modifiés + 2 nouveaux (`src/lib/timezone.ts`, `tests/rachatService.test.ts`), **rien commité ni déployé**. Commit proposé à l'utilisateur immédiatement après ce checkpoint.

## Prochaines étapes recommandées
1. Commit du travail de ce checkpoint (proposé, en attente de confirmation utilisateur)
2. Continuer la migration : `devisService.ts` (candidat #8, 20 requêtes, `RETURNING` ×2)
3. Poursuivre l'ordre établi dans `todo.md` jusqu'aux 13 services restants
4. Appliquer `lib/timezone.ts` à `ticketService.ts`/`garantiesService.ts`/`agendaService.ts`/`statsService.ts` à leur tour
5. Envisager un déploiement groupé une fois plusieurs services supplémentaires migrés (pas fait service par service jusqu'ici dans cette session)
