...
## 2026-07-25 — Refonte fiche A4 : système visuel sans aplat de couleur de marque

**Décision** : ni l'indigo actuel (`#6366f1`, en prod depuis le chantier impression checkpoint 33) ni le bleu marine/ambre de la maquette `docs/bon de réparation.pdf` (proposée initialement par la loop-engineering, checkpoint 55) ne sont retenus pour la fiche A4 (et par ricochet, les classes CSS partagées `.print-header`/`.print-table thead`/`.print-totaux-table .total-ttc`/`.print-logo-*` affectent aussi facture/devis/caisse). Retenu : accent gris ardoise unique (`#334155`), utilisé uniquement en texte/bordure, jamais en fond plein — inspiré de `docs/modele-facture.pdf` (facture SOTELI réelle, déjà imprimée par de vrais clients, aucun aplat de couleur de marque).

**Pourquoi** : contrainte explicite de l'utilisateur — économie d'encre à l'impression, aucun noir/indigo/bleu, pas de bandeau plein. L'encart "Acompte versé" (crème/ambre) est explicitement conservé, hors périmètre de cette contrainte (signal financier fonctionnel, pas un élément de branding).

**Comment appliquer** : ne pas réintroduire l'indigo (`#6366f1`)/le bleu marine sur ces classes CSS partagées à l'avenir sans repasser par une décision explicite. Détail complet, alternatives écartées et items hors périmètre (codes de sécurité jamais imprimés, titre/référence inchangés) dans `docs/superpowers/specs/2026-07-25-refonte-fiche-a4-design.md`. Chantier séparé et non lié à cette décision : le logo boutique multi-tenant (`todo.md` § Logo boutique multi-tenant), qui remplacera à terme le nom en texte affiché par défaut dans l'en-tête.

## 2026-07-25 — Collision loop-engineering / session interactive sur la même tâche backlog

**Constat** : en poussant la spec + le plan d'implémentation de la refonte A4 sur `main`/origin avant la fin de la session interactive (nécessaire pour créer le worktree isolé depuis la bonne base), le run planifié suivant de la loop-engineering (10:00, `todo.md:10`) a lu ce plan fraîchement disponible et a réinterprété une tâche qu'elle avait explicitement escaladée au run précédent (checkpoint 55, ambiguïté de branding) comme désormais implémentable — elle a commencé à éditer `public/static/js/tickets.js` **directement sur le checkout principal** (pas de worktree créé) en recopiant le contenu du plan, puis a échoué en cours de route (code sortie 1, aucun commit, aucune trace dans `.superpowers/sdd/loop-runs.md`). Le diff orphelin laissé sur `main` a été mis en sécurité par `git stash` (non perdu) pendant la session interactive qui a découvert l'incident.
