...
- [ ] Vérifier aussi `agendaService.ts` (#19) et `statsService.ts` (#20), déjà flaggés dans l'inventaire initial pour le même type de risque (`datetime('now', …)`, `julianday()`) — même traitement à leur tour
...
- [x] **personnelService.ts** — 8/9 fonctions migrées (`listEmployes`, `getEmploye`, `updateEmploye`, `desactiverEmploye`, `pointer`, `pointagesAujourdhui`, `rapportPointage`, `statutsTempsReel`). `createEmploye` reste sur `D1Database` (dépend d'`auditLog`). Tests → `mockDatabase` (36/36 ✅, `createEmploye` reste sur `mockD1`). Câblage `routes/personnel.ts` : `c.get('db')` partout sauf `createEmploye`. **Validé en local live** : 8/8 endpoints ✅ (liste, détail, update, pointage absent→en_poste, heures du jour, rapport, statuts temps réel, désactivation). Observation sans gravité : `heures_travaillees` gonflé de 2h sur cette machine (UTC+2 local) — même bug de fuseau déjà documenté (`agendaService`/`statsService`), sans impact prod (Cloudflare Workers tourne en UTC), non retraité.
  ...
  - [ ] devisService.ts — 20 requêtes, `RETURNING` ×2
  - [ ] authService.ts — sensible sécurité, SQL simple
  - [ ] stockService.ts — 21 requêtes
  - [ ] clientService.ts — 22 requêtes
  - [ ] fournisseursService.ts — 25 requêtes
  - [ ] servicesService.ts — 22 fonctions (le plus large en surface)
  - [ ] ticketService.ts — `||` ×9, calcul d'âge julianday/datetime
  - [ ] reconditionnementService.ts — `RETURNING` ×6 (le plus)
  - [ ] phoneCatalogService.ts — window function `OVER()`, **0 test existant**
  - [ ] emailService.ts — `datetime('now',…)` ×6, logique relances
  - [ ] garantiesService.ts — julianday+datetime combinés ×4, `RETURNING` ×5
  - [ ] agendaService.ts — date-math + 1 test déjà en échec préexistant (sans lien)
  - [ ] statsService.ts — le plus complexe, `||` ×9, 1 test déjà en échec préexistant (sans lien)
