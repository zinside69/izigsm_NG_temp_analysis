...
- **Une facture ne se modifie ni ne se supprime — aucun statut n'y échappe** (depuis le
  2026-09-07, ticket 003). `PUT /factures/:id` et `DELETE /factures/:id` existent uniquement pour
  répondre **405** avec un motif qui nomme l'avoir : un 404 muet laisserait croire à une erreur de
  chemin, et une immuabilité tenue par la seule *absence* de route se rouvre sans que personne
  sache qu'elle devait rester close. La règle vaut aussi pour un **brouillon**, qui ne coûte
  pourtant rien à la série (le numéro n'est attribué qu'à l'émission) : `ajouterPaiement()`
  n'accepte que les factures non verrouillées, donc **tout encaissement vit sur un brouillon** —
  le supprimer effacerait de l'argent encaissé. Le bouton 🗑 de `factures.js` est retiré : il
  appelait une route morte, et son repli hors-ligne supprimait réellement la facture du cache
  local en annonçant « supprimée ». Non-récidive : `tests/factures-immuabilite-conformite.test.ts`
  (volet statique : aucun handler `put`/`delete` sur `/factures/:id` ne doit faire autre chose
  que refuser) + deux exemptions motivées dans le garde-fou d'isolation. **Le devis, lui, reste
  modifiable** — ⊥ étendre la règle par symétrie.
- **Une facture émise est figée, y compris son identité.** `emettreFacture()` écrit `vendeur_snapshot` et `acheteur_snapshot` (JSON) : le document réimprimé doit refléter ce qui était vrai à l'émission, jamais les fiches client/boutique du jour. C'est le point de passage **unique des trois chemins brouillon → émission** (manuelle, conversion de devis, acompte) — n'ajoute jamais de figeage sur ces chemins, et ne réécris jamais ces colonnes. Une facture en brouillon n'a volontairement pas de snapshot et lit les fiches vivantes. **Second site de figeage, assumé depuis le 2026-09-04 (ticket 002)** : `caisseService.createVente()`, la vente POS naissant émise sans jamais passer par un brouillon. Il pose les six mêmes marques (`locked`, `issued_at`, `hash_nf525`, `tracking_token`, les deux snapshots) — **après** l'écriture au journal NF525, jamais dans l'`INSERT` : verrouiller avant l'existence de la chaîne laisserait, sur échec du journal, une facture immuable et orpheline donc irréparable. Ces deux sites sont les seuls ; un troisième doit être discuté avant d'être écrit.
- **Le numéro n'est attribué qu'à l'émission** (depuis le 2026-08-02, ticket 001 du chantier `.scratch/conformite-facturation/`). `nextNumero()` n'est appelé que par `emettreFacture()` — les trois chemins de création écrivent `numero = NULL` et un brouillon vit sans numéro (migration `0040` : la colonne est nullable, `UNIQUE(boutique_id, numero)` tolère plusieurs `NULL`). `emettreFacture()` **persiste le numéro avant** d'écrire au journal NF525 : un échec du chaînage ne brûle pas un second numéro, la reprise réutilise le même. Seule exception assumée : `caisseService.createVente()`, dont la vente POS est émise d'emblée. Conséquence de contrat : `createFacture()` et `convertirDevis()` renvoient `facture_numero: null` tant que la facture est brouillon — ne jamais fabriquer un numéro de repli côté client (`FAC-<id>` a été retiré de `factures.js` pour cette raison). **Vérifié en production le 2026-08-16** (checkpoint 78) : brouillon affiché « Brouillon (non numéroté) », émission `FAC-2026-00004`, second brouillon laissé tel quel puis émission `FAC-2026-00005` — compteur à 5 pour trois numéros, aucun saut, chaîne NF525 intègre et muette sur le brouillon. Ne pas rejouer cette vérification.
...
(expéditeur saisi ≠ employé, aperçu `22a33464`). Relu sur l'aperçu **puis** l'apex à chaque fois :
`sw.js` à la bonne version, `from_configure` lu par la page servie, ancienne écriture absente,
`data.success` nu à 0, `/api/notifications/stats` sans jeton → 401, `/api/health` 200 ; validé à
l'écran (bandeau vert avec l'expéditeur réel, champ de saisie vide).
