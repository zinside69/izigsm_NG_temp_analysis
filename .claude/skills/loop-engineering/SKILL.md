...
1. **Recommandation de rafraîchissement** :
   ...
   ```
   - `{"action":"skip",...}` → rien à faire côté rafraîchissement, mais poursuivre quand même au point 2 (le compteur d'échecs doit être remis à 0 même quand rien n'avait besoin d'être mis à jour).
   - `{"action":"update_failed",...}` → `detect_incremental()` a échoué (Python/graphify cassé, timeout) — poursuivre au point 2, mais avec `record-result failure` cette fois (pas `success`) : c'est une vraie panne de l'outillage, distincte du cas `skip` ci-dessus (rien à mettre à jour) — les deux ne doivent jamais être confondus, sous peine de désactiver silencieusement l'alerte de 3 échecs consécutifs.
   ...
   - `{"action":"update_no_semantic",...}` → **ne pas appeler `/graphify --update` du tout ce run**. Le pipeline `--update` de `/graphify` est tout-ou-rien (auto-détection `code_only` : soit AST seul si 100% code, soit AST+sémantique complet dès qu'un fichier non-code a changé — aucun mode intermédiaire n'est invocable de l'extérieur, vérifié dans `~/.claude/skills/graphify/SKILL.md` § "For --update"). Le rafraîchissement (AST et sémantique) est différé au prochain run où le nombre de fichiers non-code modifiés sera repassé sous le cap, ou à un `/graphify --update` manuel complet lancé par l'utilisateur en session interactive si le retard devient gênant.
2. **Enregistrer le résultat** (best-effort, ne bloque jamais) :
   ```bash
   node scripts/loop/graphify-refresh.mjs record-result success   # si l'update, le skip, ou le report (update_no_semantic) a été traité sans erreur
   node scripts/loop/graphify-refresh.mjs record-result failure   # si un vrai /graphify --update a échoué, OU si detect_incremental() lui-même a échoué (action update_failed)
   ```
